...
Deep Researchの実行時間が長大になる問題に対処するため、処理を5つのステップに分割し、Continuation Pattern（継続実行）を導入しました。

1.  **Step 1a (Primary Collection)**: 一次ソース収集
2.  **Step 1b (Article Collection)**: 調査計画生成 + 多面的記事収集
3.  **Step 2 (Evaluation)**: 信頼性評価
4.  **Step 3 (Analysis)**: ファクトチェック・時系列分析

各ステップは独立したトリガー実行として動作し、それぞれが新しい6分枠を持ちます。ステップ間は約1分間隔で自動的にチェーンされます。

### Step 1の細分化（2026/01/22追加）
当初はStep 1を一括実行していましたが、一次ソース収集（複数サイト×複数クエリ）と記事収集を合わせると6分を超える可能性があったため、さらに2段階に分割しました。これにより、一次ソースの質を維持しつつ、確実にタイムアウトを回避できるようになりました。

---

## 記事検索の抜本的改善（Gemini Grounding） ✅

RSS検索の限界（ヒット率の低さ・空振り）を克服するため、Gemini Grounding（Google Search）を利用した検索方式に移行しました。

### 従来の課題
- Google News RSSは厳密なキーワードマッチ検索のため、複雑なクエリでヒット率が低い
- カンマ区切りのクエリが正しく解釈されず、0件になることが多発

### 新方式の特徴
- GeminiにWeb検索を依頼し、結果を構造化JSONで取得
- 検索意図を汲み取った柔軟な検索が可能
- 記事のタイトル、URL、発行日、概要を確実に取得
- 失敗時は自動的にRSS検索にフォールバック

### 実装内容
- `_GeminiService.gs`: `useSearch` パラメータの追加（Google Search Groundingの有効化）
- `FetchService.gs`: `fetchByGeminiSearch` メソッドの実装とデフォルト化
