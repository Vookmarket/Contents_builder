...
### FR-3 一次ソース収集（ファクトチェック支援）
- 入力：`IntakeQueue.status=promoted`
- 処理：
  - 記事内リンク抽出（一次っぽいリンク優先）
  - 追加探索（任意）：官公庁/法令/統計/議事録/団体声明/原論文の候補を列挙
  - Claims（主張）を抽出し、Evidence候補を紐付け
- 出力：`EvidenceIndex`更新、`01_primary_sources.md`/`02_claims_check.md`生成

### FR-4 発信テーマ生成
- 入力：promoted items + screening + evidence候補
- 処理：テーマ案（タイトル案、狙い、論点、注意点、想定反論）を複数生成
- 出力：`TopicBacklog`追加（priority初期値付与）

### FR-5 ステークホルダー抽出・マッピング
- 入力：`TopicBacklog.status=draft/researching`
- 処理：利害関係者を列挙（行政/政党/業界/研究・獣医/NGO/市民/国際 等）し、関心・立場・根拠URLを付与
- 出力：`StakeholderMap`更新、`03_stakeholders.md`生成（必要ならMermaidも）

### FR-6 媒体別コンテンツ生成
- 入力：topic + evidence + stakeholder
- 共通要件：参照URL一覧、一次/二次区分、ファクト/解釈の分離
